...
let background = chrome.extension.getBackgroundPage(),
  ...
  POPUP = {},
  // as chrome 88+, the chrome.cookies.get method disallows certain properties from being passed in the CookieDetails obj
  // https://developer.chrome.com/docs/extensions/reference/cookies/#type-CookieDetails
  filterCookieForDisallowedPropsWhenGetting = (obj) => {
    delete obj.value
    delete obj.domain
    return obj
  }
...
POPUP.openColorPicker = function (companyNameSubmitted) {
  if (companyNameSubmitted) {
    ...
    window.open('https://marketolive.com/m3/apps/color-picker.html?company=' + companyNameSubmitted)
  }
}
...
window.onload = function () {
  ...
  chrome.cookies.get(filterCookieForDisallowedPropsWhenGetting(privilegesToggleCookieMarketo), function (cookie) {
    if (cookie == null || cookie.value == null) {
      ...
      LIB.setCookie(privilegesToggleCookieMarketo)
  ...
  chrome.cookies.get(filterCookieForDisallowedPropsWhenGetting(privilegesToggleCookieDesigner), function (cookie) {
    if (cookie == null || cookie.value == null) {
      ...
      document.getElementById('privilegesToggleText').innerHTML = 'Privileges Enabled'
      document.getElementById('privilegesToggle').src = '../images/toggle-on.png'
      LIB.setCookie(privilegesToggleCookieDesigner)
    } else if (cookie.value == 'true') {
      console.log('Getting: ' + cookie.name + ' Cookie for ' + cookie.domain + ' = ' + cookie.value)
      currToggleState = true
      document.getElementById('privilegesToggleText').innerHTML = 'Privileges Enabled'
  ...
  chrome.cookies.get(filterCookieForDisallowedPropsWhenGetting(saveEditsToggleCookieDesigner), function (cookie) {
    if (cookie == null || cookie.value == null) {
      ...
      LIB.setCookie(saveEditsToggleCookieDesigner)
    } else if (cookie.value == 'true') {
      console.log('Getting: ' + cookie.name + ' Cookie for ' + cookie.domain + ' = ' + cookie.value)
  ...
  chrome.cookies.get(filterCookieForDisallowedPropsWhenGetting(companyLogoCookieMarketoLive), function (cookie) {
    if (cookie == null || cookie.value == null) {
  ...
  chrome.cookies.get(filterCookieForDisallowedPropsWhenGetting(companyLogoCookieDesigner), function (cookie) {
    if (cookie == null || cookie.value == null) {
  ...
  seSubmit.onclick = function () {
    ...
    let task = '',
      addPerc = document.getElementById('addPerc').value
    ...
    let compPerc = document.getElementById('compPerc').value
  ...
  clearCache.onclick = function () {
    let clearCacheText = document.getElementById('clear-cache-text')
    ...
    chrome.browsingData.removeCache(
      ...
      function () {
        LIB.reloadTabs('*://*.marketo.com/*')
        LIB.reloadTabs('*://*.marketodesigner.com/*')
        LIB.reloadTabs('*://*.marketolive.com/*')
        LIB.reloadTabs('*://250ok.com/*')
        window.close()
      }
    )
  ...
  clear.onclick = function () {
    company.value = null
    LIB.removeCookie(companyLogoCookieMarketoLive)
    LIB.removeCookie(companyLogoCookieDesigner)
    LIB.removeCookie({
      url: mktoLiveClassicMatch,
      name: 'color',
      value: '',
    ...
    chrome.cookies.get(filterCookieForDisallowedPropsWhenGetting(saveEditsToggleCookieDesigner), function (cookie) {
      if (cookie != null && cookie.value == 'false') {
        LIB.reloadTabs(background.mktoDesignerMatchPattern)
      }
    })
...
const rawGitUrl = 'https://raw.githubusercontent.com/PMET-public/ml-ext-dist',
  // allow more flexibility by having a version specific update script
  rawGitPrefix = `${rawGitUrl}/v${localManifestVersion}`

chrome.storage.local.get(['userAttemptedUpdate'], result => {
...
checkExtUpdateAvailable().then(available => {
  if (available) {
    ...
    el.className = 'update-available',
    el.innerHTML = `<span id="mkto-update-msg">Update Available - Click Here!</span>
    <input id="mkto-update-cmd" type="text" readonly value="curl -sS ${rawGitPrefix}/update-chrome-extension.sh | bash">`
    el.addEventListener('click', ev => {
      let el = document.getElementById('mkto-update-cmd')
      el.select()
      document.execCommand('copy')
      console.log(`Copied "${el.value}" to clipboard.`)
      document.getElementById('mkto-update-msg').innerText = 'Now Paste The Cmd In the Terminal!'
