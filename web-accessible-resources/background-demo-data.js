...
chrome.cookies.get(
  ...
  function (cookie) {
    if (!cookie || !cookie.value) {
      ...
      let jobTitleX = jobTitles[Math.floor(Math.random() * jobTitles.length)]
      LIB.setCookie({
        url: mktoLiveDomainMatch,
        domain: mktoLiveUriDomain,
        name: jobTitleCookieName,
  ...
  function (cookie) {
    if (!cookie || !cookie.value) {
      ...
      if (Math.random() <= 0.25 && monthIndex < leadSources.length) {
        ...
        let index = Math.floor(Math.random() * leadSources.length)
...
function visitPage(verticalPageX, index) {
  let url = verticalPageX.url + (params ? '?' + params : '')
  chrome.tabs.create({
    url: url,
    active: false,
    selected: false,
  ...
  if (index) {
    let visitedPagesCookie = visitedPagesCookieMarketoLive
    visitedPagesCookie.value = index.toString()
    LIB.setCookie(visitedPagesCookie)
...
getVertical()
LIB.webRequest(
  'https://marketolive.com/m3/pluginv3/data/' + currVertical + '-pages-acquire.json',
  null,
  'GET',
  true,
  'json',
  function (response) {
    let verticalPages = JSON.parse(response),
        ...
        function (cookie) {
          if (cookie && cookie.value) {
            if (verticalPageX.conversionRate >= 1.0 || Math.random() <= verticalPageX.conversionRate) {
              ...
              LIB.webRequest(
                'https://marketolive.com/m3/pluginv3/data/' + currVertical + '-utm-values.json',
                null,
                'GET',
                true,
                'json',
                function (response) {
                  let utm = JSON.parse(response),
                  ...
                  params += '&utmTerm=' + encodeURIComponent(utmTermX) +
                    '&utmMedium=' + encodeURIComponent(utmMediumX) +
                    '&utmCampaign=' + encodeURIComponent(utmCampaignX) +
                    '&utmSource=' + encodeURIComponent(utmSourceX)
                }
              )
            } else {
