...
let prodExtensionId = 'onibnnoghllldiecboelbpcaeggfiohl',
...
DELIVERABILITY.removeGenericButton = function (selector, property, target) {
  // If buttons is empty, than the selector must be malformed.
  let buttons = $(selector)
  if (buttons.length == 0) {
    console.log('250ok > Error: removeGenericButton() received an empty selector: ' + selector)
    return
  }
...
if ($('#supportemail').length == 1) {
  if ($('#supportemail')[0].value == userId) {
    DELIVERABILITY.removeDeleteButtons()
    DELIVERABILITY.toRemoveSubmitButtons(extensionMinVersion)
  }
} else {
  LIB.webRequest('/app/account', null, 'GET', true, 'document', function (response) {
    let el = document.createElement('html')
    el.innerHTML = response
    email = el.querySelector('#supportemail')
    if (email && email.value == userId) {
      DELIVERABILITY.removeDeleteButtons()
      DELIVERABILITY.toRemoveSubmitButtons(extensionMinVersion)
    }
  })
