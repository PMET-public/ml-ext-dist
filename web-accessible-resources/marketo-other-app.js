  ...
  APP.waitForHeap(function () {
    ...
    switch (window.location.hostname) {
      case mktoLiveMasterRtpHostname:
        LIB.webRequest('/app/predictive/settings/user.json?', null, 'GET', true, 'json', function (response) {
          response = JSON.parse(response)
          let userId = response.body.email,
      ...
      case mktoLiveEmailInsightsHostname:
        LIB.webRequest('/api/v1/settings/user.json?', null, 'GET', true, 'json', function (response) {
          response = JSON.parse(response)
          let userId = response.result.email,
      ...
      case mktoLiveSeoHostname:
        intervalRef = window.setInterval(function () {
          if (LIB.isPropOfWindowObj('optify.user.globals') &&
            optify.user.globals.user_first_name &&
            optify.user.globals.user_last_name &&
            optify.user.globals.customer_name
          ) {
            window.clearInterval(intervalRef)
            let userName = optify.user.globals.user_first_name + ' ' + optify.user.globals.user_last_name,
              roleSubstring = userName.search(/\[[^\]]+\]/),
            ...
            if (navItem.length > 0 && navItem[0].innerHTML) {
              ...
              if (navItem[0].className.search(/ ?active ?/) != -1) {
                APP.heapEventProps({
                  ...
                  assetType: LIB.formatText(navItem[0].innerHTML)
                })
              }
              ...
              navItem[0].onclick = function () {
                APP.heapEventProps({
                  area: 'Predictive Content',
                  assetType: LIB.formatText(this.innerHTML)
                })
        ...
        case mktoWebPersonalizationPathname.test(window.location.pathname):
          if (document.getElementsByClassName('page-title').length > 0 && document.getElementsByClassName('page-title')[0].innerHTML) {
            APP.heapEventProps({
              ...
              assetType: LIB.formatText(document.getElementsByClassName('page-title')[0].innerHTML)
            })
          }
          break
      }
    ...
    case mktoEmailInsightsHostname.test(window.location.hostname):
      window.setTimeout(function () {
        ...
        if (
          ...
          let settingsButton = settingsIcon[0].parentNode.parentNode
