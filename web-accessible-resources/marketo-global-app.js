...
LIB.interceptXHR()
console.log('Marketo Global App > Running', MARKETO_EXT_VERSION)

// eslint-disable-next-line no-var
var HEAP_ANALYTICS_DEMO = 'https://marketolive.com/m3/pluginv3/heap-analytics-demo.js',
  mktoWizardDomain = '^https://app-[a-z0-9]+.marketo.com/m#'

function loadDashBoardData() {
  LIB.loadScript('https://marketolive.com/m3/pluginv3/dashboards/program-analyzer-data.js')
  LIB.loadScript('https://marketolive.com/m3/pluginv3/dashboards/social-dashboard-data.js')
  if (MktPage.savedState.custPrefix != LIB.mktoAccountStringABDemoMaster && MktPage.savedState.custPrefix != LIB.mktoAccountStringMaster) {
    LIB.loadScript('https://marketolive.com/m3/pluginv3/dashboards/nurture-dashboard-data.js')
    LIB.loadScript('https://marketolive.com/m3/pluginv3/dashboards/email-dashboard-data.js')
  }
}
...
;(async function() {
  ...
  console.log(`Marketo Global App > Account String: ${accountString}`)

  if (await LIB.isMarketoLiveInstance()) {
    console.log('Marketo Global App > Location: MarketoLive Instance')
    if (MktPage.userid.toLowerCase().search('^(mktodemolivemaster@marketo.com$|admin(.[a-z0-9]+)*@(marketolive.com$|mktodemoaccount)|mktodemoaccount[a-z0-9]*@marketo.com$|marketodemo.*@gmail.com$)') != -1) {
      console.log('Marketo Global App > User: Admin')
      LIB.loadScript('https://marketolive.com/m3/pluginv3/marketo-app-admin.js')
      return
    } else {
    ...
    if (accountString == 'mktodemoaccount408') {
      LIB.loadScript(HEAP_ANALYTICS_DEMO)
    } else if (accountString.search(`^(mktodemoaccount106|mktodemoaccount106d|mktodemolivemaster|${LIB.mktoAccountStringABDemoMaster})$`) != -1) {
      LIB.loadScript('https://marketolive.com/m3/pluginv3/heap-analytics.js')
    }

    if (accountString.search('^(mktodemoaccount106|mktodemoaccount106d)$') != -1) {
      LIB.loadScript('https://marketolive.com/m3/pluginv3/pods.js')
    }

    if (
      accountString.search(`^(mktodemolivemaster|mktodemoaccount408|mktodemoaccount106|mktodemoaccount106d|globalsales|${LIB.mktoAccountStringABDemoMaster})$`) != -1 &&
    window.location.href.search('^https://.*.marketodesigner.com/') == -1 &&
    window.location.href.search(mktoWizardDomain) == -1
    ) {
      loadDashBoardData()
    }
  } else if (await LIB.isMarketoDemoInstance()) {
    console.log('Marketo Global App > Location: Marketo Demo Instance')
    LIB.loadScript('https://marketolive.com/m3/pluginv3/marketo-demo-app.js')
    LIB.loadScript(HEAP_ANALYTICS_DEMO)
