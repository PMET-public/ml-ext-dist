...
// prevent unwanted manipulation of tout app

// eslint-disable-next-line no-var
var TOUT = TOUT || {}

TOUT.errorHandler = function (e) {
  console.error(e)
}

TOUT.parseJSONResponse = function (response) {
  try {
...
TOUT.webRequest = function (url, params, data, headers = '', method = 'GET') {
  if (!url) {
...
// get a single mock person from mockaroo
// only used when there are no more inactive people to add to a campaign
TOUT.getMockPerson = function () {
  ...
  return TOUT.webRequest('https://api.mockaroo.com/api/b96020f0', params).then(TOUT.parseJSONResponse, TOUT.errorHandler)
}
...
TOUT.getPerson = function (params) {
  ...
  return TOUT.webRequest('/people/' + params.person_id + '.json', query).then(TOUT.parseJSONResponse, TOUT.errorHandler)
}


// cur max returned is 1000 people
TOUT.getPeople = function () {
  ...
  return TOUT.webRequest('/search.json', params, '', headers, 'POST').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}

TOUT.getCampaigns = function () {
  console.log('ToutApp > Getting Campaigns')
  return TOUT.webRequest('/workflows.json', {all: true}).then(TOUT.parseJSONResponse,TOUT.errorHandler)
}

TOUT.getActiveCampaigns = function () {
  console.log('ToutApp > Getting Active Campaigns')
  return TOUT.webRequest('/workflows/active').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}

TOUT.getPeopleInCampaign = function (params) {
  console.log('ToutApp > Getting People In Campaign: ' + params.campaignId)
  return TOUT.webRequest('/workflows/' + params.campaignId + '/workflow_instances').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}

TOUT.getUncompletedTasks = function () {
  let params = {
    from: TOUT.dateOffsetInDays(new Date(), -14).toISOString(), // last 14 days
    to: TOUT.dateOffsetInDays(new Date(), 1).toISOString()
  }
  console.log('ToutApp > Getting Uncompleted Tasks')
  return TOUT.webRequest('/reminders/uncompleted.json', params).then(TOUT.parseJSONResponse, TOUT.errorHandler)
}
...
TOUT.addPerson = function (params) {
  ...
  console.log('ToutApp > Adding Person To Address Book: ' + params.first_name + ' ' + params.last_name)
  return TOUT.webRequest('/people', '', data, '', 'POST').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}

// updates the given person with the given company name
TOUT.updatePerson = function (params) {
  ...
  return TOUT.webRequest('/people/' + params.id, '', data, '', 'PUT').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}
...
TOUT.addPersonToMockGroup = function (params) {
  ...
  return TOUT.webRequest('/groups/3900413/group_members', '', data, '', 'POST').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}

// adds inactive people to the given campaign
TOUT.addPeopleToCampaign = function (params) {
  ...
  return TOUT.webRequest('/workflows/' + params.campaignId + '/workflow_instances', '', data, '', 'POST').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}

//logs a made call to complete a call task
TOUT.makeCall = function (params) {
  ...
  return TOUT.webRequest('/twilio_bridge/initiate/5555555555/5615551234', params, '', headers, 'POST').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}
...
TOUT.skipTask = function (params) {
  ...
  return TOUT.webRequest('/reminders/' + params.taskId, '', data, '', 'PUT').then(TOUT.parseJSONResponse, TOUT.errorHandler)
}
  ...
  let startTime = new Date(),
    isCsrfToken = window.setInterval(function () {
      ...
      if (
        ...
        console.log('ToutApp > CSRF-Token not detected after 5 secs or does NOT Exist')
        window.clearInterval(isCsrfToken)
      }
    })
}

// adds % (pct) inactive people to campaigns
...
// completes the given percentage (pct) of uncompleted tasks
      ...
      for (let task of response) {
        if (Math.random() < pct) {
          switch (task.remind_type) {
            case 'call':
              TOUT.makeCall({person_id: task.person_id, reminder_id: task.id})
              break
            case 'inmail':
            case 'other':
              TOUT.skipTask({taskId: task.id})
              break
          }
        }
      }
...
// removes harmful delete and save buttons in ToutApp
